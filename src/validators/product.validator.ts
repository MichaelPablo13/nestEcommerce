...
export class ProductValidator {
    ...
    static validate(body, file: Express.Multer.File, toValidate: string[]) {
        ...
        if (toValidate.includes('names') && validator.isEmpty(body.name)) {
            errors.push('Product name cannot be empty');
        }

        if (toValidate.includes('description') && validator.isEmpty(body.description)) {
            errors.push('Product description name cannot be empty');
        }
        ...
        if (toValidate.includes('imageUpdate')) {
            if (file !== undefined && !ProductValidator.imageWhiteList.includes(file.mimetype)) {
                errors.push('Invalid image format');
            }
        }
        return errors;
